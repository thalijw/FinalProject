...
if (isset($_POST['text'])){
	...
	if ($text != ""){
		...
		$time = time();
		queryMysqli("INSERT INTO messages VALUES(NULL, '$user',
			'$view' , '$pm' , $time , '$text') ");
	}
}
...
if ($view != "") {
	...
	echo <<<_END
		...
		<input type='submit' value='Post Message' id='But1'></form><br>
_END;

	if (isset($_GET['erase'])){
		$erase = sanitizeString($_GET['erase']);
		queryMysqli("DELETE FROM messages WHERE id=$erase AND recip='$user'");	
	}
	$query = "SELECT * FROM messages WHERE recip='$view' ORDER BY time DESC";
	$result = queryMysqli($query);
	$num = $result->num_rows;
